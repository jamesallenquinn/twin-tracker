...
async function snooDiscover(email, password) {
  ...
  for (const [label, path] of probes) {
    try {
      const res = await fetch("https://api-us-east-1-prod.happiestbaby.com" + path, { headers: { "Accept": "application/json", "Authorization": "Bearer " + idToken, "User-Agent": "okhttp/4.12.0" } });
      const body = await res.text();
      console.log(`[snoo] PROBE ${res.status} ${label}: ${res.ok ? body.slice(0, 700) : "(" + body.slice(0, 50).replace(/\s+/g, " ") + ")"}`);
    } catch (e) { console.log(`[snoo] PROBE ERR ${label}: ${e.message}`); }
  }
